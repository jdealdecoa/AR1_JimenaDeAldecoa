...
export class Level9 extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.tilemapTiledJSON("map_level_09", "Level9.json");
  ...
  create() {
    ...
    // --- MAPA ---
    const map = this.make.tilemap({ key: "map_level_09" });
    ...
    this.input.keyboard.on("keydown-ESC", () => {
      this.scene.launch("PauseMenu", { from: "Level9" });
      this.scene.pause();
    ...
    if (weapon && weapon.active && ball && ball.active) {
      ...
      if (this.ballsGroup && this.ballsGroup.contains(ball)) {
        ...
        if (this.ballsGroup.getChildren().length === 0 && !this._levelCompleted) {
          this._levelCompleted = true;
          console.log('Nivel 9 completado, pasando a Nivel 10');
          this.scene.start('Level10');
        }
      }
      // Play ball pop sound
