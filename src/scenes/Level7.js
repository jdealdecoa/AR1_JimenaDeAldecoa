...
export class Level7 extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.tilemapTiledJSON("map_level_07", "Level7.json");
  ...
  create() {
    ...
    // --- MAPA ---
    const map = this.make.tilemap({ key: "map_level_07" });
    ...
    this.input.keyboard.on("keydown-ESC", () => {
      this.scene.launch("PauseMenu", { from: "Level7" });
      this.scene.pause();
    ...
    if (weapon && weapon.active && ball && ball.active) {
      ...
      if (this.ballsGroup && this.ballsGroup.contains(ball)) {
        ...
        if (this.ballsGroup.getChildren().length === 0 && !this._levelCompleted) {
          this._levelCompleted = true;
          console.log('Nivel 7 completado, pasando a Nivel 8');
          this.scene.start('Level8');
        }
      }
      // Play ball pop sound
