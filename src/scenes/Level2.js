...
export class Level2 extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.tilemapTiledJSON("map_level_02", "Level2.json");
    ...
    this.load.spritesheet("hex_big", "hex_big.png", { frameWidth: 98 / 3, frameHeight: 30 });
    this.load.spritesheet("hex_mid", "hex_mid.png", { frameWidth: 52 / 3, frameHeight: 16 });
    this.load.spritesheet("hex_small", "hex_small.png", { frameWidth: 33 / 3, frameHeight: 10 });

    // --- 8. POWER-UPS (BONUS SPRITESHEET) ---
    this.load.setPath("assets/sprites/static");
    this.load.spritesheet("bonus", "bonus.png", { frameWidth: 20, frameHeight: 20 });

    // --- 9. BIRDS ---
    this.load.setPath("assets/sprites/spritesheets/enemies");
    this.load.spritesheet("bird_small", "bird_small.png", { frameWidth: 51, frameHeight: 31 });

    // --- 10. CROCODILES ---
    this.load.setPath("assets/sprites/spritesheets/enemies");
    this.load.spritesheet("crocodile", "crocodile.png", { frameWidth: 118, frameHeight: 127 });
  }
  ...
  create() {
    ...
    // --- MAPA ---
    const map = this.make.tilemap({ key: "map_level_02" });
    this.map = map;
    ...
    if (this.platformsStatic) {
      ...
      this.platformsStatic.forEachTile((tile) => {
        if (tile && tile.index > 0) tile.setCollision(true, true, true, true);
      });
    }
    ...
    if (this.platformsStatic) this.physics.add.collider(this.hero, this.platformsStatic);
    if (this.platformsBreakable) this.physics.add.collider(this.hero, this.platformsBreakable);

    // --- COLISIONES BOLAS ---
    this.wallManager.addGroupCollider(this.ballsGroup, this.bounceBall, this);
    if (this.platformsStatic) this.physics.add.collider(this.ballsGroup, this.platformsStatic, this.bounceBall, null, this);
    if (this.platformsBreakable) this.physics.add.collider(this.ballsGroup, this.platformsBreakable, this.bounceBall, null, this);

    // Overlap con el héroe
    ...
    if (this.platformsStatic) this.physics.add.collider(this.dropper.activeItems, this.platformsStatic);
    if (this.platformsBreakable) this.physics.add.collider(this.dropper.activeItems, this.platformsBreakable);

    // --- HUD ---
    this.hud = new Hud(this, { uiTop: map.heightInPixels, mode: "HARPOON" });

    // --- PAUSA ---
    this.input.keyboard.on("keydown-ESC", () => {
      this.scene.launch("PauseMenu", { from: "Level2" });
      this.scene.pause();
    ...
    this.physics.add.overlap(this.birdsGroup, this.hero, this.birdHitsHero, null, this);

    this.physics.add.overlap(this.bullets, this.birdsGroup, this.onWeaponHitsBird, null, this);
    ...
    if (this.platformsStatic) this.physics.add.collider(this.crocodilesGroup, this.platformsStatic);
    if (this.platformsBreakable) this.physics.add.collider(this.crocodilesGroup, this.platformsBreakable);

    this.physics.add.overlap(this.crocodilesGroup, this.hero, this.onPlayerHitsCrocodile, null, this);
  ...
  bounceBall(ball) {
    ...
    const now = Date.now();
    if (ball._lastBounce && now - ball._lastBounce < 60) return;
    ball._lastBounce = now;

    if (!ball._prevVelocity) ball._prevVelocity = { x: 150, y: 400 };
