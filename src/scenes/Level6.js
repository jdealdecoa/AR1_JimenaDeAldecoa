...
export class Level6 extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.tilemapTiledJSON("map_level_06", "Level6.json");
  ...
  create() {
    ...
    // --- MAPA ---
    const map = this.make.tilemap({ key: "map_level_06" });
    ...
    this.input.keyboard.on("keydown-ESC", () => {
      this.scene.launch("PauseMenu", { from: "Level6" });
      this.scene.pause();
  ...
  checkLevelCompletion() {
    if (this.ballsGroup && this.ballsGroup.getChildren().length === 0 && !this._levelCompleted) {
      this._levelCompleted = true;
      console.log('Nivel 6 completado, pasando a Nivel 7');
      this.game.audioManager.playEffect(this, 'victoria', { volume: 1 });
