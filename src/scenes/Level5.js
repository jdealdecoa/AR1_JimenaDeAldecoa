...
export class Level5 extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.tilemapTiledJSON("map_level_05", "Level5.json");
  ...
  create() {
    ...
    // --- MAPA ---
    const map = this.make.tilemap({ key: "map_level_05" });
    ...
    this.input.keyboard.on("keydown-ESC", () => {
      this.scene.launch("PauseMenu", { from: "Level5" });
      this.scene.pause();
    ...
    if (weapon && weapon.active && ball && ball.active) {
      ...
      if (this.ballsGroup && this.ballsGroup.contains(ball)) {
        ...
        if (this.ballsGroup.getChildren().length === 0 && !this._levelCompleted) {
          this._levelCompleted = true;
          console.log('Nivel 5 completado, pasando a Nivel 6');
          this.scene.start('Level6');
        }
      }
      // Play ball pop sound
