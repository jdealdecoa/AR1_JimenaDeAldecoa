...
export class Level8 extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.tilemapTiledJSON("map_level_08", "Level8.json");
  ...
  create() {
    ...
    // --- MAPA ---
    const map = this.make.tilemap({ key: "map_level_08" });
    ...
    this.input.keyboard.on("keydown-ESC", () => {
      this.scene.launch("PauseMenu", { from: "Level8" });
      this.scene.pause();
    ...
    if (weapon && weapon.active && ball && ball.active) {
      ...
      if (this.ballsGroup && this.ballsGroup.contains(ball)) {
        ...
        if (this.ballsGroup.getChildren().length === 0 && !this._levelCompleted) {
          this._levelCompleted = true;
          console.log('Nivel 8 completado, pasando a Nivel 9');
          this.scene.start('Level9');
        }
      }
      // Play ball pop sound
